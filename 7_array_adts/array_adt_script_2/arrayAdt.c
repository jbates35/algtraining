...
int binSearchADT(struct ArrayInt *arr, int val, int lo, int hi) {
  ...
  // If value hasn't been found, or has been discovered to be non-existent,
  // recurse thy value
  int mid = (lo + hi) / 2;
  int newHi, newLo;
...
int mergeSortADT(struct ArrayInt *arr) {
  ...
  // No need to do anything (This means only one value in the array is able to
  // be inspected with this call)
  if (arr->length <= 1) {
    return -1;
  }
  ...
  struct ArrayInt arrLow = {NULL, mid, mid};

  if (mid != 0)
    arrLow.A = (int *)malloc(sizeof(int) * arrLow.size);

  struct ArrayInt arrHigh = {NULL, arr->length - mid, arr->length - mid};
...
int mergeADT(struct ArrayInt *arr1, struct ArrayInt *arr2,
             struct ArrayInt *arr) { // Can't do much with a null pointer
...
int unionADT(struct ArrayInt *arr1, struct ArrayInt *arr2,
             struct ArrayInt *arr) {
  if (arr1 == NULL || arr2 == NULL)
    return -2;
...
int intersectADT(struct ArrayInt *arr1, struct ArrayInt *arr2,
                 struct ArrayInt *arr) {
  if (arr1 == NULL || arr2 == NULL)
    return -2;
...
int differenceADT(struct ArrayInt *arr1, struct ArrayInt *arr2,
                  struct ArrayInt *arr) {
  if (arr1 == NULL || arr2 == NULL)
    return -2;
