...
struct ArrayInt *findDuplicates(struct ArrayInt *arr)
{
    ...
    struct ArrayInt *hashTable = (struct ArrayInt*) malloc(sizeof(struct ArrayInt));
    initADT(hashTable, NULL, 10, 0);
    ...
    // General idea is -
    //   Simple
    //   We need a hashing array to store which values have been entered.
    for (int i = 0; i < arr->length; i++)
        setADT(hashTable, hashTable->A[arr->A[i]]+1, arr->A[i]);
    
    //  Now that we have that, we need to find what values have been placed in the hashtable twice or more
    //  and dump them into the array
    for (int i = 0; i < hashTable->length; i++)
    {
        if(hashTable->A[i] > 1)
            appendADT(missingVals, i);
    }
